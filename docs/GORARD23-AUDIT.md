...
| Composition preservation (Eq 12): `Z'(g ∘ f) = Z'(g) ∪ Z'(f) = [Z'(X), Z'(Z)] ∩ ℕ` | §2 Eq 12 | ✅ | functor/mod.rs:106 (`verify_functoriality`), tests/functoriality.rs | Per-step (`is_composable_with`) and chain-level (`is_composable_chain`) Eq 12 composability checks. |
...
| Figure 4 (vertex/edge-tagged free category, Z' with intermediate step lists) | ✅ | turing.rs + functor/mod.rs:122 (`is_composable_chain`) | |
