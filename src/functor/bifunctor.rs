...
//! - **Symmetry**: I₁ ⊗ I₂ ≅ I₂ ⊗ I₁
//!
//! ## Connection to Gorard's Paper
//!
//! The tensor structure corresponds to the "multicomputational" aspect where
//! Z': 𝒯 → ℬ must preserve the tensor product: Z'(f ⊗ g) = Z'(f) ⊕ Z'(g).
...
/// # Arguments
///
/// * `left` - The left parallel intervals to transform
/// * `right` - The right parallel intervals to transform
/// * `f` - Transformation for left component
/// * `g` - Transformation for right component
///
/// # Returns
///
/// A pair of transformed parallel intervals.
///
/// # Example
///
/// ```rust
/// use irreducible::categories::{DiscreteInterval, ParallelIntervals};
/// use irreducible::functor::bifunctor::{tensor_bimap, IntervalTransform};
///
/// let left = ParallelIntervals::from_branch(DiscreteInterval::new(0, 5));
/// let right = ParallelIntervals::from_branch(DiscreteInterval::new(10, 15));
///
/// let (new_left, new_right) = tensor_bimap(
///     left,
///     right,
///     |p| p.shift_all(5),   // Shift left intervals by 5
///     |p| p.scale_all(2),   // Scale right intervals by 2
/// );
/// ```
...
///
/// These operations apply transformations to all branches in a `ParallelIntervals`,
/// useful for bifunctor operations.
