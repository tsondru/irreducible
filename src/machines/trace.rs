//! Shared trait and utilities for irreducibility trace analysis.
//!
//! Both [`super::ExecutionHistory`] (Turing machines) and [`super::CAExecutionHistory`]
//! (cellular automata) perform near-identical logic for cycle detection, contiguity
//! checking, and complexity ratio computation. This module extracts that shared
//! behaviour into a trait and generic functions.

use crate::categories::DiscreteInterval;
use crate::functor::IrreducibilityFunctor;
...
pub struct TraceAnalysis {
    ...
    /// Whether the interval sequence under Z' is contiguous.
...
///
/// This is the generic version of `ExecutionHistory::analyze_irreducibility` and
/// `CAExecutionHistory::analyze_irreducibility`. It:
/// 1. Checks interval contiguity via [`IrreducibilityFunctor::is_sequence_irreducible`]
...
pub fn analyze_trace(trace: &impl IrreducibilityTrace) -> TraceAnalysis {
    ...
    let is_sequence_contiguous = IrreducibilityFunctor::is_sequence_irreducible(&intervals);
    let total_interval = IrreducibilityFunctor::compose_sequence(&intervals);
...
mod tests {
    ...
    /// Minimal stub for testing the generic functions in isolation.
    struct StubTrace {
    ...
    #[test]
    fn test_detect_repeats_no_repeats() {
    ...
    #[test]
    fn test_detect_repeats_single_cycle() {
        // Step 0: fp=100, Step 1: fp=200, Step 2: fp=100 (repeat of step 0)
    ...
    #[test]
    fn test_detect_repeats_multiple_cycles() {
        // fp pattern: A B A B — two repeats
    ...
    #[test]
    fn test_analyze_trace_irreducible() {
        let trace = StubTrace {
            fingerprints: vec![1, 2, 3, 4],  // 4 unique fps (initial + 3 steps)
            intervals: vec![
    ...
    #[test]
    fn test_analyze_trace_with_cycle() {
        let trace = StubTrace {
            fingerprints: vec![1, 2, 1],  // fp 1 repeats at step 2
            intervals: vec![
                DiscreteInterval::new(0, 1),
                DiscreteInterval::new(1, 2),
            ],
        ...
        assert_eq!(analysis.step_count, 2);
        // complexity_ratio = 2 / 0 → clamped to 1.0 since min_steps is 0
        assert!((analysis.complexity_ratio - 1.0).abs() < f64::EPSILON);
    }

    #[test]
    fn test_analyze_trace_non_contiguous() {
        let trace = StubTrace {
            fingerprints: vec![1, 2, 3],
            intervals: vec![
                DiscreteInterval::new(0, 1),
                DiscreteInterval::new(3, 4),  // gap
            ],
            halted: true,
        };
        let analysis = analyze_trace(&trace);
    ...
    #[test]
    fn test_repeat_detection_display() {
    ...
    #[test]
    fn test_trace_analysis_display() {
        let trace = StubTrace {
            fingerprints: vec![1, 2, 3],
            intervals: vec![
                DiscreteInterval::new(0, 1),
        ...
        assert!(s.contains("Complexity ratio: 1.000"));
    }

    #[test]
    fn test_detect_repeats_empty() {
        let repeats = detect_repeats(std::iter::empty());
        assert!(repeats.is_empty());
    }

    #[test]
    fn test_analyze_trace_empty() {
        let trace = StubTrace {
            fingerprints: vec![42],  // just initial, no steps
            intervals: vec![],
            halted: true,
        };
        let analysis = analyze_trace(&trace);
